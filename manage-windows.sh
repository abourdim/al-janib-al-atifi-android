#!/usr/bin/env bash
# ============================================================================
#  Al Janib al-Atifi Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/14-al-janib-al-atifi}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.aljanibalatifi}"
AVD_NAME="${AVD_NAME:-al_janib_al_atifi_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-janib-al-atifi-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Janib al-Atifi Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
