/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Janib Al-Atifi
...
(function() {
  ...
  const STATE = { playing:false, paused:false, mode:'page', cardIndex:0, cards:[], tabOrder:['about','cards','wisdom'], tabIndex:0, loopCount:0, loopCurrent:0, sleepTimer:null, sleepMinutes:0, duoTimeout:null, karaokeEnabled:true, autoScroll:true, duoReading:false, speed:1, pitch:1, voiceAR:null, voiceEN:null, voiceFR:null };
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.anxiety-card').forEach(el => { cards.push({ el, text: cleanText(el.textContent), type: 'wisdom' }); });
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    if (!duoText && card.type === 'wisdom' && typeof WISDOM_DATA !== 'undefined') { const ac = Array.from(card.el.parentNode.querySelectorAll('.anxiety-card')); const idx = ac.indexOf(card.el); if (idx >= 0 && WISDOM_DATA[idx] && WISDOM_DATA[idx].fr) { const a = ADHKAR_DATA[idx].fr; duoText = (a.title||'') + '. ' + (a.problem||'') + '. ' + (a.solution||''); } }
  ...
  function onSpeedChange(v){STATE.speed=parseFloat(v);const l=document.getElementById('narratorSpeedLabel');if(l)l.textContent=STATE.speed+'x';localStorage.setItem('ja-narrator-speed',STATE.speed);}
  function onPitchChange(v){STATE.pitch=parseFloat(v);const l=document.getElementById('narratorPitchLabel');if(l)l.textContent=STATE.pitch.toFixed(1);localStorage.setItem('ja-narrator-pitch',STATE.pitch);}
  ...
  function onKaraokeToggle(c){STATE.karaokeEnabled=c;localStorage.setItem('ja-narrator-karaoke',c);}
  function onAutoScrollToggle(c){STATE.autoScroll=c;localStorage.setItem('ja-narrator-autoscroll',c);}
  function onDuoToggle(c){STATE.duoReading=c;localStorage.setItem('ja-narrator-duo',c);}
  ...
  function loadSettings(){STATE.speed=parseFloat(localStorage.getItem('ja-narrator-speed'))||1;STATE.pitch=parseFloat(localStorage.getItem('ja-narrator-pitch'))||1;STATE.karaokeEnabled=localStorage.getItem('ja-narrator-karaoke')!=='false';STATE.autoScroll=localStorage.getItem('ja-narrator-autoscroll')!=='false';STATE.duoReading=localStorage.getItem('ja-narrator-duo')==='true';}
  ...
  var origRenderWisdom=window.renderWisdom;if(origRenderWisdom){window.renderWisdom=function(){origRenderWisdom();setTimeout(injectSpeakButtons,100);};}
